package com.hotstuffkotlin.ui.create
...
import com.hotstuffkotlin.databinding.FragmentCreateItemBinding
...
class CreateItemFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        val nameText = view.findViewById<TextInputEditText>(R.id.edit_name_text)
        val nameContainer = view.findViewById<TextInputLayout>(R.id.edit_name_container)
        val quantityText = view.findViewById<TextInputEditText>(R.id.edit_quantity_text)
        val quantityContainer = view.findViewById<TextInputLayout>(R.id.edit_quantity_container)
        val categoryText = view.findViewById<TextInputEditText>(R.id.edit_category_text)
        val categoryContainer = view.findViewById<TextInputLayout>(R.id.edit_category_container)
        val roomText = view.findViewById<TextInputEditText>(R.id.edit_room_text)
        ...
        val takePicture = registerForActivityResult(ActivityResultContracts.TakePicture()) {
            isSaved -> if (isSaved) {
                ...
                fos.flush()
                fos.close()
                MediaScannerConnection.scanFile(context, arrayOf(imageFile!!.path), arrayOf(com.hotstuffkotlin.ui.create.CreateItemFragment.Companion.SELECT_MIME_TYPE), null)
            }
        }
        ...
        selectPhotoButton?.setOnClickListener {
            val requestedPermission =
                com.hotstuffkotlin.ui.create.CreateItemFragment.Companion.ACCESS_PERMISSION
            val checkSelfPermission = ContextCompat.checkSelfPermission(requireActivity(), requestedPermission)
            if (checkSelfPermission == PackageManager.PERMISSION_GRANTED) {
                try {
                    selectPicture.launch(arrayOf(com.hotstuffkotlin.ui.create.CreateItemFragment.Companion.SELECT_INPUT_TYPE))
                } catch (e: Exception) {
                    e.printStackTrace()
                    Toast.makeText(context, "Error: $e", Toast.LENGTH_SHORT).show()
                }
                ...
                ActivityCompat.requestPermissions(requireActivity(), arrayOf(requestedPermission),
                    com.hotstuffkotlin.ui.create.CreateItemFragment.Companion.PERMISSION_REQUEST_CODE
                )
