...
class DatabaseHelper(val context: Context):
     ...
     * @author Cam Patten
     */
    fun addItem(item: com.hotstuffkotlin.models.Item) {
        val db = this.writableDatabase
     ...
     * @author Cam Patten
     */
    fun updateItem(item: com.hotstuffkotlin.models.Item) {
        val db = this.writableDatabase
        val cv = ContentValues()
        cv.put(COLUMN_ITEM_NAME, item.name)
        cv.put(COLUMN_ITEM_QUANTITY, item.quantity)
     ...
     * @author Cam Patten
     */
    fun addBuilding(building: com.hotstuffkotlin.models.Building) {
        val db = this.writableDatabase
        val cv = ContentValues()
     ...
     * @author Cam Patten
     */
    fun updateBuilding(building: com.hotstuffkotlin.models.Building) {
        val db = this.writableDatabase
        val cv = ContentValues()
        cv.put(COLUMN_BUILDING_NAME, building.name)
        cv.put(COLUMN_BUILDING_TYPE, building.type)
    ...
    fun getDataRange(offset: Int = 0, isSearchQuery: Boolean = false,
                     searchQuery: String? = null): ArrayList<com.hotstuffkotlin.models.Item> {
        val itemList: ArrayList<com.hotstuffkotlin.models.Item> = ArrayList()
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                val i = com.hotstuffkotlin.models.Item(
